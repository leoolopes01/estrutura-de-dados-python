# Definindo a função que terá o comportamento do bubble sort de forma decrescente, recebe a lista como parametro
def bubble_sort_decreasing(list, by):
    item = ''
    nextItem = ''
    ...
    for final in range(len(list), 0, -1):
        ...
        for current in range(0, final - 1):
            ...
            # Se o titulo da lista nessa repetição for menor que o seu sucessor
            if (item < nextItem):

                # Digo então que vou inverter o valor delas, para que a menor fique depois
...
def test_bubble_sort_decreasing(list, by):
